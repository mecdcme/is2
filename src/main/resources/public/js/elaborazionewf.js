...
$(document).ready(function () {
    ...
    table = $("#worksetTab").DataTable({
        ...
        pageLength: 15,
        serverSide: true,
        ajax: _ctx + "/rest/ws/worksetvalori/" + _idElaborazione + "/" + getParams(),
        ...
        buttons: [{
            ...
            },
            {
                ...
                action: function (e, dt, node, config) {
                    scaricaWorkSet(e, 'csv', _idElaborazione, _idProcesso);
                }
            }
        ]
    });
...
function ricercaByParams() {
    //table.ajax.reload();
    table.ajax.url(_ctx + "/rest/ws/worksetvalori/" + _idElaborazione + "/" + getParams()).load();
    return false;
}
...
function scaricaWorkSet(e, param, idelab, idproc) {
    e.preventDefault();
    window.location = _ctx + '/rest/ws/download/workset/' + param + '/' + idelab + '/' + idproc;
}