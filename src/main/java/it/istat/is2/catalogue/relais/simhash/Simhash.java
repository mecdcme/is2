	...
	public class FileRec {
		 ...
		 public class HashRec {
		    ...
		    public HashRec(int ds,int num,String value,int hashdim,int totRec,boolean wgh) throws Exception {
		       ...
		       this.hashcode=h.decodemd5(value);
		       this.hashinvert=hi.decodemd5(invert);
		        
		       /*TESTRELAIS if ((ds==0 && num==122) || (ds==1 && num==1112)) System.out.println(value+";'"+hashcode+";'"+hashinvert);*/
		    }

			
		    public void progres (int nbit,int inv) {
		       if (inv==0) hashcode = hashcode.substring(nbit)+hashcode.substring(0,nbit);
			   else        hashinvert = hashinvert.substring(nbit)+hashinvert.substring(0,nbit);
		    }
			
		    
		    public String print() {
		       return (hashcode+" : "+value+" ("+ds+":"+num+")");
		    }
		 }
		 ...
		 public static class SortHash implements java.util.Comparator {
		    ...
		    public int compare(Object boy, Object girl) {
		       String has1=((HashRec)boy).hashcode;
		       String has2=((HashRec)girl).hashcode;
