...
public class WorkSessionController {
	...
	public String apriSessione(HttpSession session, Model model, @PathVariable("id") Long id) {
		...
		if (sessionelv.getBusinessFunction().getSxArtifacts()
				.contains(new ArtifactBFunction(IS2Const.ARTIFACT_DATASET))) {
			files = new ArrayList();
			if (sessionelv.getDatasetFiles() != null) {
				session.setAttribute(IS2Const.SESSION_DATASET, true);
		...
		if (sessionelv.getBusinessFunction().getSxArtifacts()
				.contains(new ArtifactBFunction(IS2Const.ARTIFACT_RULESET))) {
