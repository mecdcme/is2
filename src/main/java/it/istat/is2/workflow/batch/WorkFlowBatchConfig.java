...
public class WorkFlowBatchConfig extends DefaultBatchConfigurer implements BatchConfigurer {
	...
	public Job doBusinessProc() {
		return (Job) jobs.get("doBusinessProc").listener(listener())
				.flow(doStep()).end().build();
	}
	...
	@Bean
	public ApplicationListener<ContextRefreshedEvent> resumeJobsListener(@Autowired JobOperator jobOperator, @Autowired JobRepository jobRepository,
