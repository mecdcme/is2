...
public interface DatasetColonnaDao extends JpaRepository<DatasetColonna, Long> {
	...
	@Query(value = "SELECT  new it.istat.rservice.dataset.domain.DatasetColonna(dc.id, dc.nome,dc.ordine,dc.filtro,dc.datasetFile,tv)  from DatasetColonna dc    LEFT JOIN   dc.tipoVariabile tv      where dc.datasetFile=:dFile  ORDER BY dc.ordine ASC")
