...
# dati grezzi
dati.sper <- read.table("dati_grezzi_2017_2016.csv",header=TRUE,sep = ",")
dati_g_7 <- dati.sper[dati.sper$PERIODON==201707,]
dati_g_8 <- dati.sper[dati.sper$PERIODON==201708,]
dati_g_9 <- dati.sper[dati.sper$PERIODON==201709,]
dati_g_12 <- dati.sper[dati.sper$PERIODON==201712,]
...
dati_g_78 <- sqldf("select a.*, b.FN_g_8_2017 
                   ...
                   where a.CODICEN=b.CODICEN") 

dati_g_789 <- sqldf("select a.*, b.FN_g_9_2017
                    from dati_g_78 as a, dati_g_9 as b 
                     where a.CODICEN=b.CODICEN") 

# <- dati_g_789[,-c(3,4,5,6,8,9)]
dati_g_789$TRIM_g_789_2017 <- round((dati_g_789$FN_g_7_2017 + dati_g_789$FN_g_8_2017 + dati_g_789$FN_g_9_2017)/3, digits = 0.1)


# dati puliti
datiFATT <- read.delim("dati_puliti_2017_2016.csv", header=TRUE,sep = ",")
datiFATT <- subset.data.frame(datiFATT, select = c(CODICEN, ATECO, PERIODON, FN2017, FN2016))

dati_p_7 <- datiFATT[datiFATT$PERIODON==201707,]
dati_p_8 <- datiFATT[datiFATT$PERIODON==201708,]
dati_p_9 <- datiFATT[datiFATT$PERIODON==201709,]
dati_p_12 <- datiFATT[datiFATT$PERIODON==201712,]

colnames(dati_p_7)[4]  <- "FN_p_7_2017"
colnames(dati_p_8)[4]  <- "FN_p_8_2017"
colnames(dati_p_9)[4]  <- "FN_p_9_2017"
colnames(dati_p_12)[4] <- "FN_p_12_2017"

colnames(dati_p_7)[5]  <- "FN_p_7_2016"
colnames(dati_p_8)[5]  <- "FN_p_8_2016"
colnames(dati_p_9)[5]  <- "FN_p_9_2016"
colnames(dati_p_12)[5] <- "FN_p_12_2016"
...
iniziali <- as.data.frame(table(dati$ATECO))
iniziali
...
esclusi <- as.data.frame(table(dati_elim$ATECO))
esclusi
